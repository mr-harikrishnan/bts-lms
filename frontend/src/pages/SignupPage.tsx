...
export const SignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Presence check
    if (!trimmedName || !trimmedEmail || !trimmedCollege || !trimmedDistrict || !trimmedState || !password || !confirmPassword) {
        ...
        <main className="flex-1 flex items-center justify-center px-4 py-12 relative overflow-hidden">
          ...
          <div className="w-full max-w-xl bg-white/95 backdrop-blur-xl rounded-3xl p-7 sm:p-10 shadow-[0_20px_50px_-15px_rgba(15,23,42,0.07),0_1px_3px_rgba(15,23,42,0.04)] border border-slate-200/80">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full h-12 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-semibold text-sm shadow-md hover:shadow-lg hover:shadow-slate-900/10 active:scale-[0.99] transition-all flex items-center justify-center gap-2 group cursor-pointer disabled:opacity-60"
                >
                  {isSubmitting ? (
                    <>
            ...
            <p className="mt-6 text-center text-xs text-slate-400">
              By registering, you agree to DLABS{" "}
              <Link to="/terms" target="_blank" className="underline hover:text-slate-600 transition-colors">
                Terms of Service
              </Link>{" "}
              and{" "}
              <Link to="/privacy" target="_blank" className="underline hover:text-slate-600 transition-colors">
                Privacy Policy
              </Link>
              .
            </p>
