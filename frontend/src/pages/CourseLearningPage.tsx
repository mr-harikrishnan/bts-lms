...
export const CourseLearningPage: React.FC = () => {
  ...
  useEffect(() => {
    if (course && user.isLoggedIn && !enrolled) {
      if (course.price === 0) {
        ...
        navigate(`/checkout/${course._id}`, { replace: true });
      }
    }
  }, [user.isLoggedIn, enrolled, course, enrollCourse, navigate]);
  ...
  const currentModuleNumber = activeItem?.moduleNumber || "M1";
  ...
  const isCurrentCompleted = currentLesson ? completedLessonIds.includes(currentLesson._id) : false;
